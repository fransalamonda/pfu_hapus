...
class Wbi extends BaseController
{
    ...
    public function index()
    {
        try {
            ...
            return view("pages/content_" . $_SESSION['app_role_id'] . "", $data_page);
        } catch (\Exception $exc) {
            if ($exc->getCode() == 2)
                return redirect()->to('/');
            else
        ...
        if ($this->request->isAJAX()) {
            try {
                ...
                foreach ($respon_data as $item) {
                    ...
                    //$newDate = $item['Inspection_Date'];
                    $newDate = date("d/m/Y", strtotime($item['Inspection_Date']));
                    if ($item['Equipment_ID'] == '001_BC11') {
                        $nostr1++;
                        $str1 .= "<tr class='font-tabel-isidata' >";
                        $str1 .= "<td   title=''>Need Action</td>";
                        $str1 .= "<td   title=''>" . $newDate . "</td>";
                        $str1 .= "<td   title=''>" . $item['Equipment_ID'] . "</td>";

                        $str1 .= "<td   title=''>" . $item['Condition'] . "</td>";
                        $str1 .= "<td   title=''></td>";
                        $str1 .= "<td  class=''><a href='javascript:void(0)' " . $tmped . " title='Open Case'><h7 class='mt-3 mb-0 ' ><small></small>Open Detail</h7></a></td>";
                        $str1 .= "</tr>";
                    }
                    if ($item['Equipment_ID'] == '001_BC1') {
                        $nostr2++;
                        $str2 .= "<tr class='table-active' >";
                        $str2 .= "<td  class='text' title=''>Need Action</td>";
                        $str2 .= "<td  class='text' title=''>" . $newDate . "</td>";
                        $str2 .= "<td  class='text' title=''>" . $item['Equipment_ID'] . "</td>";

                        $str2 .= "<td  class='text' title=''>" . $item['Condition'] . "</td>";
                        $str2 .= "<td  class='text' title=''></td>";
                        $str2 .= "<td  class=''><a href='javascript:void(0)' " . $tmped . " title='Open Case'><h7 class='mt-3 mb-0 ' ><small></small>Open Detail</h7></a></td>";
                        $str2 .= "</tr>";
                    }
                    if ($item['Equipment_ID'] == '001_HP4') {
                        $nostr3++;
                        $str3 .= "<tr class='table-active' >";
                        $str3 .= "<td  class='text' title=''>Need Action</td>";
                        $str3 .= "<td  class='text' title=''>" . $newDate . "</td>";
                        $str3 .= "<td  class='text' title=''>" . $item['Equipment_ID'] . "</td>";

                        $str3 .= "<td  class='text' title=''>" . $item['Condition'] . "</td>";
                        $str3 .= "<td  class='text' title=''></td>";
                        $str3 .= "<td  class=''><a href='javascript:void(0)' " . $tmped . " title='Open Case'><h7 class='mt-3 mb-0 ' ><small></small>Open Detail</h7></a></td>";
                        $str3 .= "</tr>";
                    }
                }
                ...
                if ($nostr3 == '0') {
                    ...
                    $str3 .= "<img class='table-no-data-icon' 
                                src='" .   $basurl . "/public/assets/images/Vector_ceklist.png' height='18'></Br>";
                    $str3 .= "<p class='table-no-data-bolt'><b>Well Done</b><p></Br></p></Br>";
                    $str3 .= "</div></td>";
                    $str3 .= "</tr>";
                }
        ...
        if ($this->request->isAJAX()) {
            try {
                ...
                if (!$respon_data = cache($cache_dept)) {
                    ...
                    $url_dept = 'http://localhost/api_wbi/api_data/depart';
                ...
                foreach ($respon_data['dept'] as $d) {
                    $str_dept .= "<option required='' value='" . $d['id'] . "'>";
                    $str_dept .= $d['status_name'] . "</option>";
                }
                ...
                //staf pic
                $str_staf = "<option value=''> - Choose - </option>";
                foreach ($respon_data['staffs'] as $p) {
                    $str_staf .= "<option value='" . $p['staf_id'] . "'>";
                    $str_staf .= $p['staf_name'] . "</option>";
                }
